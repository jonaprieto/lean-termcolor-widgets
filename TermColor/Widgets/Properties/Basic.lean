...
theorem zero_total_is_complete
    : (progressBar { width := 8 } { total := 0 }).plainText = "[━━━━━━━━] 100%"
    := by
  decide

theorem progress_clamps_overflow
    : (progressBar { width := 4 } { current := 9, total := 4 }).plainText = "[━━━━] 100%"
    := by
  decide

theorem progress_rounds_down
    : (progressBar { width := 5 } { current := 1, total := 3 }).plainText = "[━────] 33%"
    := by
  decide
...
theorem empty_spinner_is_safe
    : (spinnerFrame { frames := [] } 10).plainText = ""
    := by
  decide
...
theorem shimmer_preserves_text
    : (shimmer {} { frame := 10 } (Text.styled "Think" Style.bold)).plainText = "Think"
    := by
  decide

theorem status_renders_marker_and_message
    : (renderStatus .warning (Text.plain "slow connection")).plainText = "[warn] slow connection"
    := by
  decide
...
theorem text_input_renders_fixed_width
    : (renderTextInput { width := 4 } { value := "ab", cursor := 1 }).plainText = "[ab  ]"
    := by
  decide

theorem text_input_keeps_cursor_visible
    : (renderTextInput { width := 4 } { value := "abcdef", cursor := 5 } true).plainText = "[cdef]"
    := by
  decide

theorem text_input_body_is_unframed
    : (textInputBody { width := 4 } { value := "ab", cursor := 1 } false).plainText = "ab  "
    := by
  decide
...
theorem slider_renders_value
    : (renderSlider { width := 4, min := 0, max := 8 } { value := 4 }).plainText = "[━━──] 4"
    := by
  decide
